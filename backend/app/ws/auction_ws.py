...
async def auction_room(websocket: WebSocket, auction_id: int):
    ...
    await websocket.send_json(
        {"type": "state", "data": build_state(db, auction).model_dump(mode="json")}
    )
    ...
    try:
        while True:
            ...
            if message_type not in ("bid", "pass"):
            ...
            if message_type == "pass":
                ...
                if user.id not in item.passed_user_ids:
                    ...
                    db.commit()
                await manager.broadcast_state(auction_id, build_state(db, auction))
